...
import {
  getMedicalImagesTableColumns,
  MedicalImagesExploreTableRowRenderer,
} from 'pages/MedicalImagesExplore/components/MedicalImagesExploreTableGrid/MedicalImagesExploreTableGrid';
...
import {
  IMedicalImageData,
  IMedicalImageRunData,
  IMedicalImagesExploreAppConfig,
  IMedicalImagesExploreAppModelState,
} from 'types/services/models/medicalImagesExplore/medicalImagesExploreAppModel';
import {
  IAppModelState,
  ISelectConfig,
  ISelectOption,
...
function getConfig(): IMedicalImagesExploreAppConfig {
  return {
    grouping: {
      row: [],
      reverseMode: {
...
function setDefaultAppConfigData(recoverTableState: boolean = true) {
  const defaultConfig: Partial<IMedicalImagesExploreAppConfig> = {};
  const searchParam = new URLSearchParams(window.location.search);

  const grouping: IMedicalImagesExploreAppConfig['grouping'] =
  ...
  const images: IMedicalImagesExploreAppConfig['images'] =
...
function getImagesData(
  ...
  const configData: IMedicalImagesExploreAppConfig | undefined =
    model.getState()?.config;
  if (queryString) {
...
function processData(data: any[]): {
  data: IMetricsCollection<IMedicalImageData>[];
  params: string[];
  runProps: string[];
  ...
  let contexts: string[] = [];
  data?.forEach((run: IMedicalImageRunData) => {
    ...
    run.traces.forEach((trace: any) => {
      ...
      trace.values.forEach((stepData: IMedicalImageData[], stepIndex: number) => {
        stepData.forEach((image: IMedicalImageData) => {
...
function setModelData(rawData: any[], configData: IMedicalImagesExploreAppConfig) {
  ...
  const tableColumns = getMedicalImagesTableColumns(
    params,
    groupingSelectOptions,
    data[0]?.config,
...
function updateModelData(
  configData: IMedicalImagesExploreAppConfig = model.getState()!.config!,
  shouldURLUpdate?: boolean,
): void {
  const { data, params, runProps, highLevelParams, contexts, selectedRows } =
...
function getFilteredGroupingOptions(
  grouping: IMedicalImagesExploreAppConfig['grouping'],
): string[] {
  const {
    // reverseMode,
    isApplied,
  ...
  const groupingSelectOptions:
    | IMedicalImagesExploreAppModelState['groupingSelectOptions']
...
function getDataAsTableRows(
  processedData: IMetricsCollection<IMedicalImageData>[],
  paramKeys: string[],
  isRawData: boolean,
  config: IMedicalImagesExploreAppConfig,
  ...
  tableData.forEach((metricsCollection: IMetricsCollection<IMedicalImageData>) => {
    ...
    if (metricsCollection.config !== null && !isRawData) {
      rows[groupKey!].data = MedicalImagesExploreTableRowRenderer(
        rows[groupKey!].data,
        onModelRunsTagsChange,
        {},
        true,
...
function onExportTableData(e: React.ChangeEvent<any>): void {
  const { data, params, config, groupingSelectOptions } =
    model.getState() as IMedicalImagesExploreAppModelState;
  const tableData = getDataAsTableRows(
    data,
    params,
    true,
  ...
  const tableColumns: ITableColumn[] = getMedicalImagesTableColumns(
    params,
    groupingSelectOptions,
    data[0]?.config,
...
function getQueryStringFromSelect(
  selectData: IMedicalImagesExploreAppConfig['select'],
...
function toggleSelectAdvancedMode() {
  const modelState: IMedicalImagesExploreAppModelState | any = model.getState();
...
const onImageSizeChange = _.throttle((value: number) => {
  ...
  if (configData?.images) {
    ...
    updateURL(config as IMedicalImagesExploreAppConfig);
    model.setState({
      config,
    });
  }
...
const medicalImagesExploreAppModel = {
  ...model,
  initialize,
  getImagesData,
  abortRequest,
...
export default medicalImagesExploreAppModel;
